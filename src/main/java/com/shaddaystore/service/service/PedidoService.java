...
import com.shaddaystore.service.entity.dto.GenerarPedidoDTO;
import com.shaddaystore.service.entity.dto.PedidoConDetallesDTO;
import com.shaddaystore.service.repository.DetallePedidoRepository;
import com.shaddaystore.service.repository.PedidoRepository;
...
public class PedidoService {
  ...
  public GenericResponse<List<PedidoConDetallesDTO>> devolverMisCompras(int idCli) {
    ...
    return new GenericResponse(OPERACION_CORRECTA, RPTA_OK, "Petición resuelta.", dtos);
  }
  ...
  public ResponseEntity<Resource> exportInvoice(int idCli, int idOrden) {
    ...
    if (optPedido.isPresent()) {
      try {
        ...
        final File imgOpcional = ResourceUtils.getFile("classpath:images/logo_negocio.jpg");

        final JasperReport report = (JasperReport) JRLoader.loadObject(file);

        final HashMap<String, Object> parameters = new HashMap<>();
        parameters.put("nombreCliente", pedido.getCliente().getNombreCompletoCiente());
        parameters.put("direccionEnvio", pedido.getCliente().getDireccionEnvio());
        parameters.put("imgLogo", new FileInputStream(imgLogo));
        parameters.put("imgOpcional", new FileInputStream(imgOpcional));
